...
async def echo(websocket: ServerConnection):  # websocket: ServerConnection — это объект соединения с клиентом
...
async def main():
    server = await websockets.serve(echo, "localhost", 8765)  # Запускаем сервер, который слушает соединения на localhost:8765 и использует echo в качестве обработчика сообщений
